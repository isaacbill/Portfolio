...
const ResumeCard = ({ title, subTitle, result, des }) => {
  return (
    <div className="w-full h-96 group flex"> {/* Set a fixed height for all ResumeCards, e.g., h-96 */}
      ...
      <div className="w-full bg-black bg-opacity-20 hover:bg-opacity-30 duration-300 rounded-lg p-4 lgl:px-10 flex flex-col justify-center gap-6 lgl:gap-10 shadow-shadowOne">
        ...
        <div className="text-sm md:text-base font-medium text-gray-400 group-hover:text-gray-300 duration-300 h-52 overflow-y-scroll" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {des.split('\n').map((line, index) => (
